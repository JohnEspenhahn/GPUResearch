...
float k2(float temp, float nH, float nH2) { 
	float kH = 1.2e-9*exp(-5.24e4/temp);
	float kL = 1.12e-10*exp(-7.035e4/temp);
	
	float log_temp = log(temp/1.0e4);
	float ncr = pow(4.0 - 0.416*log_temp - 0.327*log_temp*log_temp, 10);
	
	return STEP_TIME * kH*pow(kH/kL, -1.0/(1+nH/ncr));
}
float k3(float temp, float nH, float nH2) { 
	float kH = 1.3e-9*exp(-5.33e4/temp);
	float kL = 1.18e-10*exp(-6.95e4/temp);
	
	float log_temp = log(temp/1.0e4);
	float ncr = pow(4.845 - 1.3*log_temp + 1.62*log_temp*log_temp, 10);
